...
def score_probabilities(y_true: Iterable[int], probabilities: Iterable[float]) -> dict[str, float]:
    ...
    return {
        ...
        "log_loss": -sum(y * math.log(p) + (1 - y) * math.log(1 - p) for y, p in clipped) / len(clipped),
